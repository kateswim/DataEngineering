...
# CHANGED FROM: cut -f1,4 -d"#" (for web log)
# TO: awk -F: 'NF >= 6 {print $1":"$3":"$6}' (for /etc/passwd)
...
# CHANGED FROM: tr "[a-z]" "[A-Z]" (uppercase conversion)
# TO: sed 's/:/,/g' (colon to comma conversion for CSV format)
...
# CHANGED FROM: zip log.zip capitalized.txt (create zip archive)
# TO: cp (copy file for CSV output)
load = BashOperator(
    task_id='load',
    bash_command=f'''
    echo "Inside Load"
...
# NEW: Added check task (wasn't in original)
check = BashOperator(
    task_id='check',
    bash_command=f'''
    echo "Inside Check"
...
# Task pipeline
# CHANGED FROM: download >> extract >> transform >> load
# TO: extract >> transform >> load >> check (removed download, added check)
